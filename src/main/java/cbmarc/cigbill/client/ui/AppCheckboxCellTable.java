...
// TODO implements HasSelectionHandlers<T>?
public class AppCheckboxCellTable<T> extends Composite implements
		HasClickHandlers, SelectionChangeEvent.Handler, EventTranslator<T> {

	private static Binder binder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, AppCheckboxCellTable<?>> {
	}
	...
	public AppCheckboxCellTable() {
		...
		checkboxHeader.addValueChangeHandler(new ValueChangeHandler<Boolean>(){
			...
			public void onValueChange(ValueChangeEvent<Boolean> event) {
				for(T t:cellTable.getVisibleItems()) {
					selectionModel.setSelected(t, event.getValue());
				}
	...
	public SelectAction translateSelectionEvent(CellPreviewEvent<T> event) {
		...
		if ("click".equals(nativeEvent.getType())) {
			...
			} else if (!"input".equals(target.getTagName().toLowerCase())
					&& event.getColumn() != 0) {
